...
QuicClientNetworkHelper::QuicClientNetworkHelper(
...
bool QuicClientNetworkHelper::CreateUDPSocketAndBind(
...
void QuicClientNetworkHelper::CleanUpUDPSocket(SocketFd fd) {
  CleanUpUDPSocketImpl(fd);
  fd_address_map_.erase(fd);
}

void QuicClientNetworkHelper::CleanUpAllUDPSockets() {
...
void QuicClientNetworkHelper::CleanUpUDPSocketImpl(SocketFd fd) {
  if (fd != kInvalidSocketFd) {
...
QuicPacketWriter* QuicClientNetworkHelper::CreateQuicPacketWriter() {
...
void QuicClientNetworkHelper::SetClientPort(int port) {
  fd_address_map_.back().second =
      QuicSocketAddress(GetLatestClientAddress().host(), port);
}

QuicSocketAddress QuicClientNetworkHelper::GetLatestClientAddress()
    const {
...
SocketFd QuicClientNetworkHelper::GetLatestFD() const {
  if (fd_address_map_.empty()) {
    return -1;
  }
...
void QuicClientNetworkHelper::ProcessPacket(
...
SocketFd QuicClientNetworkHelper::CreateUDPSocket(
...
bool QuicClientNetworkHelper::BindInterfaceNameIfNeeded(SocketFd fd) {
  QuicUdpSocketApi api;
